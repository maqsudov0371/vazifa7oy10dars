...
from .models import User, Course, Category, Tag, Header, HeaderImage, Service, ServiceBook, Teachers
...
class ServiceView(View):
    def get(self, request):
        ...
        latest_services = Service.objects.order_by('-id')[:3]  # Eng so'nggi 3 ta xizmatni olish
        header = Header.objects.first()
        header_images = HeaderImage.objects.filter(header=header)
        ...
        context = {
            "services": services,
            "header": header,
            "latest_services": latest_services,  # Eng so'nggi xizmatlarni templatega yuborish
